## Streamlit app for predicting movie ratings
##   based on analysis of user provided text
...
@st.cache_resource
def load_encoder(fpath=FPATHS['data']['ml']['encoder']):
    return joblib.load(fpath)
...
X_to_pred = st.text_input("### Enter text to predict here:", 
                          value="I loved the directing and the musical score!")


# Update the function to decode the prediction
def make_prediction(X_to_pred, clf_pipe=clf_pipe, encoder=encoder):
    # Get Prediction
...
## Place the 3 checkboxes side-by-side
